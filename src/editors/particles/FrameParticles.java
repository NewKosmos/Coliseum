...
public class FrameParticles extends Standard {
	...
	private void addNameField() {
		...
		nameField.getDocument().addDocumentListener(new DocumentListener() {
			...
			private void textUpdate() {
				if (((ExtensionParticles) KosmosEditor.getEditorType()).particleTemplate != null) {
					((ExtensionParticles) KosmosEditor.getEditorType()).particleTemplate.setName(nameField.getText());
				}
			}
		});
				...
				if (returnValue == JFileChooser.APPROVE_OPTION) {
					...
					if (selectedFile.contains("res/particles")) {
						...
						if (fileName.contains(".png")) {
							TextureObject texture = TextureFactory.newBuilder().setFile(new MyFile(MyFile.RES_FOLDER, "particles", fileName)).create();

							if (((ExtensionParticles) KosmosEditor.getEditorType()).particleTemplate != null) {
								((ExtensionParticles) KosmosEditor.getEditorType()).particleTemplate.setTexture(texture);
							}
						} else {
							((ExtensionParticles) KosmosEditor.getEditorType()).loadFromParticle = fileName.replace(".particle", "");
						}
					} else {
	...
	private void addTextureRowBox() {
		...
		rowSlider.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				...
				if (((ExtensionParticles) KosmosEditor.getEditorType()).particleTemplate != null && ((ExtensionParticles) KosmosEditor.getEditorType()).particleTemplate.getTexture() != null) {
					((ExtensionParticles) KosmosEditor.getEditorType()).particleTemplate.getTexture().setNumberOfRows(reading);
				}
			}
		});

		FlounderEvents.addEvent(new IEvent() {
			private TextureObject currentTexture = null;
			...
			public boolean eventTriggered() {
				...
				TextureObject newTexture = ((ExtensionParticles) KosmosEditor.getEditorType()).particleTemplate.getTexture();
	...
	private void addScaleSlider() {
		...
		scaleSlider.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				...
				if (((ExtensionParticles) KosmosEditor.getEditorType()).particleTemplate != null) {
					((ExtensionParticles) KosmosEditor.getEditorType()).particleTemplate.setScale(reading / 100.0f);
				}
			}
		});
	...
	private void addLifeSlider() {
		...
		lifeSlider.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				...
				if (((ExtensionParticles) KosmosEditor.getEditorType()).particleTemplate != null) {
					((ExtensionParticles) KosmosEditor.getEditorType()).particleTemplate.setLifeLength(reading / 10.0f);
				}
			}
		});
		...
		resetButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (JOptionPane.showConfirmDialog(frame,
						...
						JOptionPane.YES_NO_OPTION,
						JOptionPane.QUESTION_MESSAGE) == JOptionPane.YES_OPTION) {
					((ExtensionParticles) KosmosEditor.getEditorType()).particleSystem.removeParticleType(((ExtensionParticles) KosmosEditor.getEditorType()).particleTemplate);
					((ExtensionParticles) KosmosEditor.getEditorType()).particleTemplate = new ParticleTemplate("testing", null, 1.0f, 1.0f);
					((ExtensionParticles) KosmosEditor.getEditorType()).particleSystem.addParticleType(((ExtensionParticles) KosmosEditor.getEditorType()).particleTemplate);
					KosmosParticles.clear();
					nameField.setText(((ExtensionParticles) KosmosEditor.getEditorType()).particleTemplate.getName());
	...
	private void save() {
		...
		saveButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (((ExtensionParticles) KosmosEditor.getEditorType()).particleTemplate != null) {
					KosmosParticles.save(((ExtensionParticles) KosmosEditor.getEditorType()).particleTemplate);
				}
			}
		});

...
public class InstanceGemRed extends Entity {
	private static final ModelObject MODEL = ModelFactory.newBuilder().setFile(new MyFile(FlounderEntities.ENTITIES_FOLDER, "gemRed", "model.obj")).create();
	private static final TextureObject TEXTURE = TextureFactory.newBuilder().setFile(new MyFile(FlounderEntities.ENTITIES_FOLDER, "gemRed", "diffuse.png")).setNumberOfRows(1).create();
	private static final TextureObject TEXTURE_GLOW = TextureFactory.newBuilder().setFile(new MyFile(FlounderEntities.ENTITIES_FOLDER, "gemRed", "glow.png")).setNumberOfRows(1).create();
	...
	public InstanceGemRed(ISpatialStructure<Entity> structure, Vector3f position, Vector3f rotation) {
		...
		colliders.add(new Pair<>(new Sphere(0.75f), new Vector3f(0.0f, 0.2f, 0.0f)));
